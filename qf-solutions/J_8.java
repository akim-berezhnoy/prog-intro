...
public class Problem_J {
    public static void main(String[] args) {
        ...
        byte[][] way = new byte[n][n];
        for (int i = 0; i < n; i++) {
            String line = sc.next();
            for (int j = 0; j < n; j++) {
                way[i][j] = Byte.parseByte(String.valueOf(line.charAt(j)));
            }
        }
        ...
        for (int node_from = 0; node_from < n; node_from++) {
            for (int node_to = node_from+1; node_to < n; node_to++) {
                if (way[node_from][node_to]%10 != 0) {
                    for (int node = node_to+1; node < n; node++) {
                        way[node_from][node] = Byte.parseByte(String.valueOf(Math.abs(way[node_from][node] - way[node_to][node])%10));
                    }
                }
            }
        }
